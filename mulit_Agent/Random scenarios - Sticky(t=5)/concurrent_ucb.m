function [ aoiExperienced, timesArmHasBeenPlayed, tx_power_SR, tx_power_RD, instantaoiAfterAction_A, instantaoiAfterAction_B, regretExperiencedPerWlan, meanRewardPerAction] = ...
    concurrent_ucb( wlans, varargin )
...
%       * initialEpsilon - initial exploration coefficient
    ...
    timesArmHasBeenPlayed = ones(2, K);% Store the times an action has been played in each WN
    ...
    cumulativeRewardPerWlanPerArm = zeros(2, K);     % Initialize the cumulative reward obtained by each WLAN for each arm
    meanRewardPerWlanPerArm = zeros(2, K);           % Initialize the mean reward obtained by each WLAN for each arm
    % random scenarios h
%             channel_coefficient = 1/sqrt(2)*(randn(2,4) + j*randn(2,4));
    ...
    for k = 1 : K
        ...
        while iteration <= 21
            ...
            for wlan_i = 1 : 2
                cumulativeRewardPerWlanPerArm(wlan_i, order_actions(wlan_i,k)) = rw(wlan_i);
                aoiExperienced(iteration, wlan_i) = -rw(wlan_i);
                meanRewardPerWlanPerArm(wlan_i, order_actions(wlan_i,k)) = rw(wlan_i);
            end
    ...
    while(iteration < totalIterations + 1) 
        ...
        for wlan_i = 1 : 2
           if mod(iteration, 10) == 1 || iteration == 1
                ...
                ix = find(allCombs(:,1) == previousAction(order(wlan_i)) & allCombs(:,2) == currentAction(order(wlan_i)));
