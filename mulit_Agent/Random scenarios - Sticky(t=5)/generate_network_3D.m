...
% printMap=1
    ...
    for j=1:nWlans 
        ...
        elseif (topology == 'line')
            wlans(j).x = 5;%j*((MaxX-2)/nWlans);
            wlans(j).y = 5 - (MaxY/2+ MaxRangeY); %MaxY/2;
            wlans(j).z = MaxZ/2; 
            wlans(j).xd = 5;%j*((MaxX-2)/nWlans);
            wlans(j).yd = MaxY/2+ MaxRangeY; %MaxY/2;
            wlans(j).zd = MaxZ/2; 
        ...
        switch topology
            ...
            case 'line'
                ...
                elseif stasPosition == 2 % SAFE
                    wlans(j).xn = j*((MaxX-2)/nWlans); %wlans(j).x;
                    wlans(j).yn = MaxY/2; %wlans(j).y + MaxRangeY;
                    wlans(j).zn = MaxZ/2; %wlans(j).z;
                elseif stasPosition == 3 % EXPOSED
