function [ aoiExperienced, timesArmHasBeenPlayed, tx_power_SR, tx_power_RD, instantaoiAfterAction_A, instantaoiAfterAction_B] = ...
    concurrent_egreedy( wlans, initialEpsilon, varargin )
...
%       * initialEpsilon - initial exploration coefficient
    ...
    rewardPerArm = zeros(2, K).*(-10);  
    
    % random scenarios h
%     channel_coefficient = 1/sqrt(2)*(randn(1,4) + j*randn(1,4));
    ...
    cumulative_reward_per_action = zeros(2, K);
...
%             channel_coefficient = 1/sqrt(2)*(randn(2,4) + j*randn(2,4));
        ...
        for i = 1:2
           if mod(iteration, 5) == 1 || iteration == 1
                ...
                selectedArm(i) = select_action_egreedy(rewardPerArm(i,:), epsilon); 
        ...
        for i = 1:2
            ...
            rewardPerArm(i,selectedArm(i)) = rw(i);
